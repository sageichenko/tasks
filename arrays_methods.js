...
// средний возраст
let averageAge = users.reduce( (averAge, current) =>  averAge + current.age/users.length, 0);
...
let averRateGroupByAge = new Object(null);
for (let key in ageGroups) {
    averRateGroupByAge[key] = ageGroups[key].reduce( (averRate, current) =>  averRate + current.rate/ageGroups[key].length, 0);
}
...
averRateGroupByAge = [];
for (let key in ageGroups) {
    averRateGroupByAge.push(ageGroups[key].reduce( (averRate, current) =>  averRate + current.rate/ageGroups[key].length, 0));
}
